...
segment_revenue = df.groupby('customer_segment')['purchase_amount'].sum().reset_index()
...
st.altair_chart(
    ...
    .mark_bar()
    .encode(
        x=alt.X('payment_method', sort=None),
        y=alt.Y('count')
    )
)
...
state_revenue = df.groupby('state')['purchase_amount'].sum().sort_values(ascending=False).head(10)
st.altair_chart(
    alt.Chart(state_revenue.reset_index())
    .mark_bar()
    .encode(
        x=alt.X('state', sort=None),
        y=alt.Y('purchase_amount')
    )
)
